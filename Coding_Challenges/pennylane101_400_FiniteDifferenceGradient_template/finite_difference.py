...
def my_finite_diff_grad(params):
    ...
    for i in range(len(params)):
        ...
        # create delta for this iteration
        delta = params[i]/1000
        
        # create extra array to that can be changed 
        delta_params = params
        
        # calculate the gradient
        delta_params[i] = params[i] + delta
        pos_cost = cost(delta_params)
        
        delta_params[i] = params[i] - delta
        neg_cost = cost(delta_params)
        
        gradients[i] = (pos_cost - neg_cost) / delta
