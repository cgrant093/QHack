...
def hamiltonian_coeffs_and_obs(graph):
    ...
    def ni_obs(i):
        ni_op = qml.PauliZ(i) + qml.Identity(i)
        return qml.Hermitian(qml.utils.sparse_hamiltonian(ni_op).real.toarray(), i)
    
    # add single body terms
    obs = [ni_obs(i) for i in range(num_vertices)]
    coeffs = [-1/2 for i in range(num_vertices)]
    
    #adding neighbor terms
    edge_matrix, num_edges = edges(graph)
    
    for i in range(num_vertices):
        for j in range(i+1, num_vertices):
            if (edge_matrix[i, j]):
                obs.append(ni_obs(i) @ ni_obs(j))
                coeffs.append(u/4)
...
def train_circuit(num_vertices, H):
    ...
    init_params = np.random.normal(0, np.pi, num_vertices, requires_grad = True)
    params = init_params
    E = 0
