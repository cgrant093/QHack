...
def distance(A, B):
    ...
    def U_matrix_norm(V):
        norm_const = np.sqrt(V[0]**2 + V[1]**2)
        U_matrix = np.array([[V[0], -V[1]],[V[1], V[0]]])
        return (U_matrix / norm_const)
    ...
    def circuit():
        # state prep
        qml.QubitUnitary(U_matrix_norm(A), wires=1)
        qml.QubitUnitary(U_matrix_norm(B), wires=2)
    ...
    AB_prob = circuit()#2*(circuit() - 0.5)
    
    distance = np.sqrt(2*(1 - np.sqrt(AB_prob)))
