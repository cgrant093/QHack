...
def check_simplification(op1, op2):
    ...
    # QHACK
    check_simp_boole = True
    
    for i in range(len(op1)):
        if check_simp_boole == False:
            return False
        
        if (op1[i] != "I") and (op2[i] != "I") and (op2[i] != op1[i]):
            check_simp_boole = False

    return check_simp_boole
...
def compression_ratio(obs_hamiltonian, final_solution):
    ...
    ratio = len(final_solution) / len(obs_hamiltonian)
